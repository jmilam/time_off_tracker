class TimeOffRequest < ApplicationRecord
	...
	scope :upcoming_week, -> {where('date_start >= ? AND date_end <= ? AND approved = ? AND cancelled = ?', Date.today.next_week.beginning_of_week, Date.today.next_week.end_of_week, true, false)}
